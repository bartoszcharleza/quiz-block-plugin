...
// Run this code when document fully loaded
document.addEventListener("DOMContentLoaded", function () {
  const divsToUpdate = document.querySelectorAll(".paying-attention-update-me");

  // Iterate each selected div
  divsToUpdate.forEach(function (div) {
    // Parse JSON data from the div
    const data = JSON.parse(div.querySelector("pre").innerHTML);

    // Render the Quiz component in the div, passing parsed data as props
    ReactDOM.render(<Quiz {...data} />, div);

    // Remove the class to avoid another updates
    div.classList.remove("paying-attention-update-me");
  });

  // Quiz component
  function Quiz(props) {
    // Track correct/incorrect answer status
    const [isCorrect, setIsCorrect] = useState(undefined);
    const [isCorrectDelayed, setIsCorrectDelayed] = useState(undefined);

    // Use useEffect for delayed actions for correct/incorrect answers
    useEffect(() => {
      if (isCorrect === false) {
        setTimeout(() => {
